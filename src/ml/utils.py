...
def train_surprise_model(n_epochs=20, verbose=True):
    ...
    cv_result = cross_validate(model, loaded_data, measures=['RMSE', "MAE"], cv=4, verbose=True)
    trainset = loaded_data.build_full_trainset()
    model.fit(trainset)
    acc = get_model_acc(trainset, model)
    acc_label = int(100*acc)
    model_name = f"model-{acc_label:0.2f}"
    export_model(model, model_name=model_name, model_type='surprise')
...
def load_model(model_type='surprise', model_ext='pkl'):
    ...
    if path_latest.exists():
        print(f"Loading model from {path_latest}")
        with open(path_latest, 'rb') as f:
            model_data = pickle.load(f)
            model = model_data.get('model')
    else:
        print(f"No model found at {path_latest}")
